...
연결 시험 `sram-page-reset-integration-v1`은 동결된 출력 FIFO·페이지 관리·실제 연결된 burst writer를 묶고, SRAM 내용은 리셋 뒤에도 유지하는 모델을 사용했다. 활성 `/WE` 10ns 뒤 공통 리셋을 넣어 기존 35ns 모델에서 미완료 쓰기 1건을 만들었다. 그 뒤 새 합성 프레임을 보내 사용 바이트 21,764개가 현재 세대로 다시 기록되었음을 확인했고, 완료/게시 각 1회였다. 리셋 직후 불완전 페이지는 공개되지 않았다. 이 결과는 **한 리셋 위상·한 합성 프레임의 디지털 모델**이며 실제 게임 CPU, SNES 버스 서비스, 물리 SRAM, 클록 정지까지의 연결 동작을 검증하지 않는다. 짧은 `/WE` 펄스도 해결되지 않았다.
